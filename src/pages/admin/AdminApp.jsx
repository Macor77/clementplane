...
function AdminHome() {
  ...
  const eventLabels={organization_dashboard_viewed:'Accueil OF',trainer_dashboard_viewed:'Accueil Formateur',trainer_search_viewed:'Recherche formateurs',missions_viewed:'Missions',planning_viewed:'Planning',availability_viewed:'Disponibilités',availability_share_viewed:'Partage des disponibilités',proposals_viewed:'Propositions',discover_viewed:'Découvrir Formaplane'};
  return <div><div className="admin-heading"><div><span className="admin-kicker">Pilotage interne</span><h1>Dashboard Admin</h1><p>Adoption, activité et croissance de Formaplane.</p></div><div className="admin-period">Temps réel · fenêtres glissantes 7 j / 30 j</div></div>
  <section className="admin-section"><div className="admin-section-title"><div><span className="admin-kicker">Adoption</span><h2>Base utilisateurs</h2></div></div><div className="admin-cards admin-cards--primary"><StatCard label="Utilisateurs" value={a.total} detail={`+${a.new_30d||0} sur 30 jours`} explanation={`${a.total||0} personnes possèdent un compte Formaplane.`}/><StatCard label="Utilisateurs OF" value={a.organization_profiles} explanation={`${a.organization_profiles||0} utilisateurs disposent d’un accès à un espace OF.`}/><StatCard label="Utilisateurs formateurs" value={a.claimed_trainers} explanation={`${a.claimed_trainers||0} utilisateurs disposent d’un profil formateur.`}/></div><div className="admin-inline-info"><strong>Dont {a.double_profiles||0} double{a.double_profiles===1?'':'s'} profil{a.double_profiles===1?'':'s'}</strong><span>{a.double_profiles||0} utilisateur{a.double_profiles===1?'':'s'} utilise{a.double_profiles===1?'':'nt'} Formaplane à la fois comme OF et comme formateur.</span></div><div className="admin-structure-info"><strong>{o.total||0} organismes enregistrés</strong><span>{o.total||0} structures disposent actuellement d’un espace OF dans Formaplane. +{o.new_30d||0} sur 30 jours.</span></div></section>
...
export default function AdminApp() {
  ...
  return <div className="admin-shell"><aside className="admin-sidebar"><img src="/brand/formaplane-logo-light.svg" alt="Formaplane" /><span className="admin-sidebar-label">ADMINISTRATION</span><nav><NavLink end to="/admin">⌂ <span>Dashboard</span></NavLink><NavLink to="/admin/crm">✉ <span>Mini-CRM</span></NavLink><NavLink to="/admin/utilisateurs">♙ <span>Utilisateurs</span></NavLink><NavLink to="/admin/organismes">▣ <span>Organismes</span></NavLink><NavLink to="/admin/communications">✦ <span>Nouveautés</span></NavLink></nav><div className="admin-sidebar-footer"><a href="/">← Revenir à Formaplane</a><button onClick={signOut}>Se déconnecter</button></div></aside><main className="admin-main">{content}</main></div>;
}
