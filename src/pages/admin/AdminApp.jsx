...
function AdminHome() {
  ...
  const a=stats.accounts||{}, o=stats.organizations||{}, m=stats.missions||{}, av=stats.availability||{}, e=stats.emails||{}, su=stats.support||{}, u=stats.usage||{}, c=stats.curves||{}, pwa=stats.pwa||{};
  const responseRate=pct(m.responses,m.proposals_sent), acceptanceRate=pct(m.accepted,m.responses);
  const eventLabels={organization_dashboard_viewed:'Accueil OF',trainer_dashboard_viewed:'Accueil Formateur',trainer_search_viewed:'Recherche formateurs',missions_viewed:'Missions',planning_viewed:'Planning',availability_viewed:'Disponibilités',availability_share_viewed:'Partage des disponibilités',proposals_viewed:'Propositions',discover_viewed:'Découvrir Clementplane',app_opened:'Ouverture de Clementplane',tutorial_interaction:'Tutoriels'};
  ...
  <section className="admin-section"><div className="admin-section-title"><div><span className="admin-kicker">Application mobile / PWA</span><h2>Adoption et mode d’accès</h2></div><span className="admin-section-note">Ouvertures authentifiées</span></div><div className="admin-cards admin-cards--primary"><StatCard label="Utilisateurs ayant lancé la PWA" value={pwa.pwa_users_total} detail={`${pct(pwa.pwa_users_total,pwa.accounts_total)}% des utilisateurs`} explanation="Un utilisateur est compté dès qu’il ouvre au moins une fois Clementplane depuis l’application installée."/><StatCard label="OF ayant utilisé la PWA" value={pwa.pwa_organization_users} detail={`${pct(pwa.pwa_organization_users,pwa.organization_users_total)}% des utilisateurs OF`} explanation="Utilisateurs disposant d’un accès OF ayant lancé Clementplane en mode PWA."/><StatCard label="Formateurs ayant utilisé la PWA" value={pwa.pwa_trainer_users} detail={`${pct(pwa.pwa_trainer_users,pwa.trainer_users_total)}% des formateurs`} explanation="Utilisateurs disposant d’un profil formateur ayant lancé Clementplane en mode PWA."/><StatCard label="Utilisateurs PWA actifs / 30 j" value={pwa.pwa_users_30d} explanation="Utilisateurs distincts ayant ouvert la PWA au moins une fois au cours des 30 derniers jours."/></div><div className="admin-access-split"><div><span>📱 PWA · 30 derniers jours</span><strong>{pct(pwa.pwa_openings_30d,pwa.openings_30d)}%</strong><small>{pwa.pwa_openings_30d||0} ouverture{pwa.pwa_openings_30d===1?'':'s'}</small></div><div><span>🌐 Navigateur · 30 derniers jours</span><strong>{pct(pwa.browser_openings_30d,pwa.openings_30d)}%</strong><small>{pwa.browser_openings_30d||0} ouverture{pwa.browser_openings_30d===1?'':'s'}</small></div></div><p className="admin-help">Le suivi mesure les ouvertures de Clementplane par des utilisateurs authentifiés, et non uniquement les saisies d’e-mail/mot de passe. Aucun modèle de téléphone ni identifiant matériel n’est enregistré.</p></section>
