-- Formaplane — Sprint 11.8.1
-- Après annulation d'une mission :
-- - les formateurs disposant d'un compte repassent automatiquement
--   en "dispo" sur les dates de la mission annulée ;
-- - les options rendues indisponibles par une affectation conflictuelle
--   sont réconciliées ;
-- - le statut "cancelled" est autorisé pour une revalidation interrompue.
...
begin
  ...
  /*
   * Un compte formateur ayant une mission annulée est remis
   * automatiquement en "Disponible" sur chacune des dates concernées.
   *
   * Le statut "mission" n'est pas stocké dans trainer_availability :
   * il était calculé depuis mission_formateurs. Ici nous écrivons donc
   * explicitement "dispo" pour matérialiser la disponibilité retrouvée.
   */
  ...
  select
    ...
    coalesce(ta.note, ''),
    now()
  from tmp_formaplane_cancelled_trainers ct
  join public.trainers t
    on t.id = ct.trainer_id
  ...
  /*
   * Une affectation annulée peut aussi libérer des options qui avaient
   * été automatiquement marquées indisponibles à cause du conflit.
   */
  for v_target in
    select trainer_id
    from tmp_formaplane_cancelled_trainers
  loop
...
revoke all
on function public.cancel_mission_with_trainers(
  uuid,
  text,
  text
)
